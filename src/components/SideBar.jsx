...
const SideBar = () => {
  ...
  return (
    <div className="relative z-50">
      <div
        className={`bg-sideBarColor rounded-r-[40px]   pt-5 pb-5 px-4 h-screen    fixed top-0 left-0 transition-all duration-75  ${
          sideBarOpen ? "w-60" : "w-[75px]"
        } `}
      >
        <div className="min-h-[calc(100vh-235px)]  max-h-[calc(100vh-235px)]  overflow-y-auto scrollbar-none">
          <div
            className={`flex  ${
        ...
        >
          ...
          <div
            className={`mt-5 ml-2.5  fixed bottom-3 left-0 ${
              sideBarOpen ? "w-[213px]" : "w-[55px]"
            } `}
          >
            <GoPremiumButton isHidden={isHidden} sideBarOpen={sideBarOpen} />
          </div>

          {isResourcesOpen && (
            <div className="fixed bottom-0 rounded-br-[40px]  left-0 w-60 h-[400px] bg-gradient-to-t z-10 from-[rgba(0.4,0,0,0.6)] to-transparent backdrop-blur-xs pointer-events-none"></div>
          )}
        </div>
      </div>
    </div>
